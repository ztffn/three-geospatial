...
the colour; the specular gloss (`metalness`, `roughness`) is driven by a constant
**up** normal (≈ lit from the surface), NOT the flank — a flank normal lit the
sun-facing side and shadowed the other, so fish flickered bright/dark and read as
not double-sided. The up normal lights both faces the same regardless of heading,
so no emissive cheat is needed. It renders as an **opaque alpha-cutout** (alphaTest
