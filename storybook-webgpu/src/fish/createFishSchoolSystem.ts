...
export function createFishSchoolSystem(
  ...
  const material = createFishMaterial(texture, textureFrames, tint, u)
 ...
 * axis. The atlas variant (`aMap`) feeds the albedo, and the flank normal drives
 * the specular gloss (flipped per face since the sprite is double-sided).
 */
function createFishMaterial(
  ...
  // Light the flat sprite from a constant UP normal (≈ from the surface) rather
  // than its flank: a flank normal lit the sun-facing side and shadowed the
  // other (looked not double-sided), while the up normal lights both faces the
  // same regardless of heading. Specular gloss then reads as a top sheen.
